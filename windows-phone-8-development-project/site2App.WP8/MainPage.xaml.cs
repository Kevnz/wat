...
namespace site2App.WP8
{
    public partial class MainPage : PhoneApplicationPage
    {
        ...
        {
            ((ApplicationBarIconButton)ApplicationBar.Buttons[2]).IsEnabled = (ShellTile.ActiveTiles.FirstOrDefault(x => x.NavigationUri.ToString().Contains("MainPage.xaml")) == null);
        }
        ...
        private void BuildLocalizedApplicationBar()
        {
                ...
                (_webConfig.Share != null && _webConfig.Share.IsEnabled))
            {
                ...
                else
                {
                    ...
                    // $TODO: Wizard needs to make sure the number of buttons does not exceeds 4
                    ...
                    || !String.IsNullOrEmpty(_webConfig.Settings.FeedbackUrl))
                {
                    ...
                    // Launch in IE
                    ApplicationBarMenuItem appBarButtonIE = new ApplicationBarMenuItem(AppResources.LaunchInIE);
                    appBarButtonIE.Click += appBarButtonIE_Click;
                    ApplicationBar.MenuItems.Add(appBarButtonIE);
        ...
        void appBarButtonIE_Click(object sender, EventArgs e)
        {
        ...
        void appBarButtonPin_Click(object sender, EventArgs e)
        {
            ...
            if (appElement != null)
            {
                std.Title = appElement.Attribute("Title").Value;
            }
                    ...
                    //else // Otherwise check for file types
                    {
                        ...
                        if (!handledExtension) // Are we launching a media file?
                        {
                            ...
                            if (!e.Cancel)
                            {
                                ...
                                if (e.Uri.OriginalString.ToLower().Contains(cnbcVideoUri) || e.Uri.OriginalString.Contains(escapedBloombergVideoUri))
                                {
                                    MediaPlayerLauncher MPL = new MediaPlayerLauncher();
                                    MPL.Media = e.Uri;
                                    MPL.Show();

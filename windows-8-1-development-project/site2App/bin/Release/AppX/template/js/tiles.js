...
<<<<<<< HEAD
    var setupLiveTile, setupPinning, pinHandler, pinComplete;
=======
    var setupLiveTile, checkSiteforMetaData, processLiveTileMetaTags, setupTileFeed,
<<<<<<< HEAD
        setupPinning, pinHandler, pinComplete;
>>>>>>> release-0.6
=======
...
<<<<<<< HEAD
        var n, updater, address, recurrence,
            urisToPoll = [];

        if (WAT.config.livetile.enabled !== true) {
            return;
        }
...
>>>>>>> release-0.6
            ...
            for (n = 0; n < 5; ++n) {
                address = "http://discourse.azurewebsites.net/FeedTile.ashx?index=" +
                          String(n) +
<<<<<<< HEAD
                          "&url=" + encodeURIComponent(WAT.config.livetile.tilePollFeed);
                try {
                    urisToPoll.push(new Windows.Foundation.Uri(address));
                } catch (e) {
                    // broken address, never mind
                    console.error("Unable to load tilePollFeed", e);
                    break;
                }
            }
